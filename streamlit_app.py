...
base_url = os.getenv("BACKEND_URL",
                     "https://11f8-34-124-236-242.ngrok-free.app")
...
def mbti_test():
  ...
  pages = [{
      ...
      "questions": [{
          ...
          "id": "q4",
          "text": "I prefer to think out loud rather than reflect internally.",
          "dimension": "E-I"
      }, {
          "id": "q5",
...
def loading_screen():
  ...
  strengths_by_type = {
      ...
      "INFJ": [
          ...
          "Long-term vision and planning", "Strong personal values as guidance"
      ],
      "INFP": [
      ...
      "ISFJ": [
          "Practical support and assistance", "Detailed memory of what works",
          "Dedication to completing tasks"
      ],
      "ESTJ": [
  ...
  results = {
      ...
      mbti_type,
      "description":
      descriptions.get(mbti_type, f"{mbti_type} - Analytical and thoughtful"),
      "strengths":
      json.dumps(
          random.sample(
              strengths_by_type.get(mbti_type, strengths_by_type["INTJ"]), 3)),
      "weaknesses":
      json.dumps(
          random.sample(
...
def results_view():
  ...
  if st.session_state.logged:
    ...
    try:
      ...
      st.error(f"Failed to update profile with new personality results: {e}")
        ...
        try:
          ...
          r = requests.post(
              ...
              json={
                  ...
                  "personality_results":
                  st.session_state.personality_results  # Include full results
              },
              timeout=10)
          r.raise_for_status()
  ...
  with col1:
    if st.button("Login", use_container_width=True) and username and password:
      try:
        ...
        st.session_state.user_id = data["user_id"]
        st.session_state.user = username
        st.session_state.personality_type = data.get("personality_type",
                                                     "INTJ")
        st.session_state.logged = True
...
#       # If this is a new session with no messages, send an automatic welcome message
#       if not st.session_state.messages:
#         try:
#           # Get session info to check if it's new
#           session_url = f"{base_url}/api/sessions/{session_id}"
#           r = requests.get(
#               session_url,
#               headers={"Authorization": f"Bearer {st.session_state.token}"},
#               timeout=10)
#           r.raise_for_status()
#           session_data = r.json()["session"]

#           # If session has no messages, send a welcome message
